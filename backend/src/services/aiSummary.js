...
export async function generatePatientProblemSummary({
  ...
  const formatMedications = allActiveMeds.length > 0 
    ? allActiveMeds.map(m => `• ${m.name} (${m.dosage}, ${m.frequency})`).join('\n')
    : 'None';

  const formatRedAlerts = redAlerts.length > 0
    ? `\n\nRED ALERTS / CONTRAINDICATIONS:\n${redAlerts.map(a => `• [${a.severity || 'CRITICAL'}] ${a.title || a.drug || a.finding}: ${a.warning || a.actionableProtocol}`).join('\n')}`
    : '';

  const ayurResponses = structuredQA.filter(q => q.section === 'Ayurveda Dashavidha Pariksha');
  const clinicalResponses = structuredQA.filter(q => q.section !== 'Ayurveda Dashavidha Pariksha');

  const ayurText = ayurResponses.length > 0 
    ? ayurResponses.map(q => `• ${q.question}: ${q.answer}`).join('\n')
    : 'No Ayurveda assessment recorded.';

  const clinicalText = clinicalResponses.length > 0
    ? clinicalResponses.map(q => `• ${q.question}: ${q.answer}`).join('\n')
    : 'No clinical Q&A recorded.';

  let historyOfPresentIllness = `PATIENT DETAILS:
• Age: ${age || 'Not specified'}
• Genetic Disorders: ${geneticDiseases.join(', ') || 'None reported'}
• Lifelong Health Issues: ${permanentDiseases.join(', ') || 'None reported'}

CHIEF PROBLEM REPORTED: ${problemText.toUpperCase()}

CLINICAL ASSESSMENT (BASED ON PROBLEM):
${clinicalText}

DASHAVIDHA PARIKSHA (AYURVEDA ASSESSMENT):
${ayurText}

ACTIVE MEDICATION:
${formatMedications}${formatRedAlerts}`;

  let clinicalImpressionForDoctor = `Clinical Impression based on reported problem: ${problemText}. Cross-verify reported symptoms with Dashavidha Pariksha findings. Note permanent conditions (${permanentDiseases.join(', ') || 'None'}) and genetic predispositions (${geneticDiseases.join(', ') || 'None'}).`;
