...
import { generateDynamicQuestions } from './services/dynamicQuestions.js';
...
// ----------------------------------------------------
app.post('/api/patient/problem-summary', async (req, res) => {
  const { patientId, problemText, languageCode, inputMode, structuredQA, apiKey } = req.body;
  if (!patientId) {
  ...
  try {
    const aiSummary = await generatePatientProblemSummary({
      problemText,
  ...
  return res.json({
    ...
    return res.status(500).json({ success: false, message: 'Failed to generate problem summary.', error: err.message });
  }
});

app.post('/api/patient/dynamic-questions', async (req, res) => {
  const { problemText, languageCode, apiKey } = req.body;
  if (!problemText) {
    return res.status(400).json({ success: false, message: 'Problem text is required.' });
  }

  const dynamicData = await generateDynamicQuestions({ problemText, languageCode, apiKey });
  if (dynamicData && dynamicData.questions && dynamicData.questions.length > 0) {
    return res.json({ success: true, category: dynamicData });
  }

  return res.json({ success: false, message: 'Could not generate dynamic questions.' });
});

// Explicitly Save a Generated Problem Summary & Persist to Dashboard History
app.post('/api/patient/save-problem-summary', async (req, res) => {
  const { patientId, problemText, aiSummary, inputMode, apiKey } = req.body;
  if (!patientId) {
    return res.status(400).json({ success: false, message: 'Patient ID is required.' });
  }
  const patient = sharedStore.getPatientById(patientId);
  ...
  const generatedSummary = aiSummary || await generatePatientProblemSummary({
  ...
  const newLog = {
    id: generatedSummary?.reportId || ('prob-' + Date.now()),
    patientId: patId,
    timestamp: new Date().toISOString(),
    inputMode: inputMode || 'CLINICAL_SUMMARY',
    language: 'en-IN',
    rawTranscript: problemText || generatedSummary?.chiefComplaint || 'Patient Reported Problem',
    structuredQA: [],
...
// --------------------------------------------------
// MONOLITHIC STATIC SERVING FOR PRODUCTION DEPLOYMENT
// --------------------------------------------------
const patientDistPath = path.join(__dirname, '..', '..', 'apps', 'patient', 'dist');
const doctorDistPath = path.join(__dirname, '..', '..', 'apps', 'doctor', 'dist');

// Serve doctor app static assets under /doctor-app to avoid collision
app.use('/doctor-app', express.static(doctorDistPath));
app.get('/doctor-app/*', (req, res) => {
  res.sendFile(path.join(doctorDistPath, 'index.html'));
});

// Serve patient app on root
app.use(express.static(patientDistPath));
app.get('*', (req, res) => {
  res.sendFile(path.join(patientDistPath, 'index.html'));
});
